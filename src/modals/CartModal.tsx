...
const CartModal: FC<CartModalProps> = ({ open, setOpen }) => {
  ...
  useEffect(() => {
    ...
    const fetchCartData = async () => {
      ...
      if (data) {
        ...
        setFilePath(data[0]?.socks.sock_image_url);
        setCartDatas(data || []);
        console.log("Image url", data[0]?.socks.sock_image_url);
  ...
  async function removeItem(id: any) {
    ...
    if (error) {
      console.log("Cart item cannot be deleted", error);
    }
